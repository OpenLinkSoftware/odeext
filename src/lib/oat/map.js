/*
 *  $Id: map.js,v 1.18.4.7 2012/01/16 19:07:12 pvk Exp $
 *
...
OAT.Map = function(something, provider, optionsObject, specificOptions) {
	...
	markerIcon:OAT.Preferences.imagePath+"markers/default.png",	/* icon used as map marker */
	markerIconSize:[18,41],	/* icon size */
...
//	    self._fixMarkers ();
	...
	case OAT.Map.TYPE_G3:
	    if (!marker.__OAT_infoWindow)
		marker.__OAT_infoWindow = new google.maps.InfoWindow({content:elm});
	    marker.__OAT_infoWindow.open(self.obj, marker);
	    break;
	...
	for (var i=0;i<self.markers.length;i++) {
	    ...
	    case OAT.Map.TYPE_G3: if (marker.__OAT_infoWindow) marker.__OAT_infoWindow.close(); break;
	...
	case OAT.Map.TYPE_G3:
	    marker.setMap(null)
	    if (marker.__OAT_infoWindow) marker.__OAT_infoWindow.closeInfoWindow();
	    break;
	case OAT.Map.TYPE_Y:
	    self.obj.removeOverlay(marker);
	...
	case OAT.Map.TYPE_Y:
	    ...
	    break;
	case OAT.Map.TYPE_MS:
	    marker.__window.closeInfoWindow();
	    marker.Hide();
	    break;
	case OAT.Map.TYPE_G:
	    marker.closeInfoWindow();
