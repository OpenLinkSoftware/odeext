/*
 *  $Id: rssreader.js,v 1.5.4.7 2012/01/16 19:07:13 pvk Exp $
 *
...
OAT.RSSReader = function(div,options) {
	...
	this.display = function(xmlText) {
		...
		if (self.options.showTitle) { 
			var title = OAT.Dom.create("h3",{className:"oat_rss_header"});
			var link = OAT.Dom.create("a");
			link.href = data.link;
			link.innerHTML = data.title;
			OAT.Dom.append([title,link],[self.div,title]);
		}
		var body = OAT.Dom.create("ul",{className:"oat_rss_body"});
