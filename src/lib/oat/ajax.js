/*
 *  $Id: ajax.js,v 1.13.4.8 2012/03/19 13:15:16 pvk Exp $
 *
...
OAT.AJAX = {
    AUTH_NONE:0,
    AUTH_BASIC:1,
    AUTH_DEFAULT:2, // Leave it to the browser to decide
    TYPE_TEXT:0,
    TYPE_XML:1,
    httpError:1,
    SHOW_NONE:0,
	...
	if (typeof(OAT.Preferences.ajax_http_headers)!="undefined"
	    && OAT.Preferences.ajax_http_headers!=null) 
	  {
	    try {
	      var h = OAT.Preferences.ajax_http_headers;

	      if (isArray(h)) {
	        for(var i=0; i < h.length; i++)
	          xhr.setRequestHeader(h[i].header, h[i].value);
	      }
	    } catch(e) {}
	  }
	...
	var stat_ctr = OAT.Dom.create ('div', {className: 'oat_xhr_status_dlg'});
	var stat_thr = OAT.Dom.create ('img', {className: 'oat_xhr_status_thr'});
	stat_thr.src = OAT.Preferences.imagePath + "arrow_anim_throbber.gif";
	var stat_msg = OAT.Dom.create ('div', {className: 'oat_xhr_status_msg'});
	...
	var err_ctr = OAT.Dom.create ('div', {className: 'oat_xhr_err_dlg'});
	var err_msg = OAT.Dom.create ('div', {className: 'oat_xhr_err_msg'});
        err_msg.innerHTML = "Problem retrieving data, status=" + xhr.getStatus() + ".";
	
	// XXX make details toggle
	
	var err_btn_1 = OAT.Dom.create("input",{type:'button'});
	err_btn_1.value = "Details";
	err_btn_1.onclick = function() { window.alert(xhr.getResponseText()); }
	
	var err_btn_2 = OAT.Dom.create("input",{type:'button'});
	err_btn_2.value = "Continue";
	
	OAT.Dom.append([err_ctr,err_msg,err_btn_1,err_btn_2]);
	
	var error_dlg = new OAT.Dialog("XHR error", err_ctr,
				       { width:240, modal:0, zIndex:1001, resize:0,buttons: false});

	err_btn_2.onclick = function() { OAT.AJAX.endNotify(OAT.Preferences.showAjax); error_dlg.close();}
	...
	this.open = function(method, target, async) {
	    try {
		if (self.options.auth == OAT.AJAX.AUTH_DEFAULT && 
		    self.options.user && 
		    self.options.password)
		    self.obj.open(method, target, async, self.options.user, self.options.password);
		else
