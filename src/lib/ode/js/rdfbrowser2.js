/*
 *  $Id: rdfbrowser2.js,v 1.42.4.9 2012/01/16 19:07:16 pvk Exp $
 *
...
OAT.RDFBrowser2 = function(div,optObj) {
	...
	this.getIE_opt = function (id, def) {
            try {
              var val = ode_prefs[id];
...
/****	
	...
	if (OAT.Browser.isChrome()) {
	    ...
	    this.options.http_headers = prefs.getCharPref("extensions.ode.headers");
	    OAT.Preferences.ajax_http_headers = this.options.http_headers;
...
****/
	if (OAT.Browser.isMozilla || OAT.Browser.isWebKit || OAT.Browser.isOpera) { //&& navigator.userAgent.match(/Chrome/)){
	    ...
	    if ("ep" in obj)
	       this.options.endpoint = obj.ep;
	    if ("vt" in obj)
	       this.options.viewertype = obj.vt;
	    if ("rp" in obj) {
	       this.options.rdfproxy = obj.rp;
               OAT.Dereference.options.endpoint = this.options.rdfproxy;
	    }
	    if ("ps" in obj)
	       proxyservice = obj.ps;
	    ...
	    if ("headers" in obj) {
	    	this.options.http_headers = OAT.JSON.deserialize(obj.headers);
	        OAT.Preferences.ajax_http_headers = this.options.http_headers;
	    }
...
//	    this.options.imagePath = '../images/';
	    self.is_xpi = false;
	} 
	else if (OAT.Browser.isIE 
	         || !!navigator.userAgent.match(/msie 9/i) 
	         || !!navigator.userAgent.match(/msie 10/i) 
	         || !!navigator.userAgent.match(/msie 11/i)
	         || !!navigator.userAgent.match(/msie 12/i))
	{

            var ie_storage = new ActiveXObject("openlinksw.ODE_helper.1");

	    var proxyservice = ie_storage.getOption("proxyservice","virtuoso");

	    var _headers = [];
	    var i = 0;
	    while(true) {
	       var _name = ie_storage.getOption("h_name"+i,"");
	       var _val = ie_storage.getOption("h_val"+i,"");
	       if (_name.length == 0) 
	         break;
	       _headers.push({header:_name, value:_val});
	       i++;
	    }

	    if (_headers.length > 0) {
	        OAT.Preferences.ajax_http_headers = this.options.http_headers = _headers;
	    }

	    this.options.endpoint = ie_storage.getOption("sparqlgenendpoint","http://linkeddata.uriburner.com/sparql")
	    this.options.viewertype = ie_storage.getOption("viewertype","builtin");
	    this.options.rdfproxy = ie_storage.getOption("proxygenendpoint","http://linkeddata.uriburner.com/about?url=");
	...
	this.store.loadFromURL = function() {
		...
		if (typeof(obj.view) != "object") { /* array of uris */
		    self.load_indicator_add (self.store.addURL(obj.view), obj.view);
		}
		return;
	    }
	        ...
	        || !!navigator.userAgent.match(/msie 12/i)) 
	    {
              obj.uri = [];
              var ie_storage = new ActiveXObject("openlinksw.ODE_helper.1");
              var lst = ie_storage.getURLs().split(",");
              for(var i=0; i <lst.length;i++)
                obj.uri.push(decodeURIComponent(lst[i]));
	    }
